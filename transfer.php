...
$username="root";
$password="";
$con=mysqli_connect($server,$username,$password,"shubham");
if(!$con){
...
$sql = "SELECT Balence FROM shb WHERE name='$sender'";
$result = $con->query($sql);

if ($result->num_rows > 0) {
  // output data of each row
          while($row = $result->fetch_assoc()) {
 if($amount>$row["Balence"] or $row["Balence"]-$amount<100){
  $location='Details.php?user='.$sender;
  header("Location: $location&message=transactionDenied");
 }
else{
    $sql = "UPDATE `shb` SET Balence=(Balence-$amount) WHERE Name='$sender'";
...
if($flag==true){
$sql = "UPDATE `shb` SET Balence=(Balence+$amount) WHERE name='$receiver'";
...
if($flag==true){
    $sql = "SELECT * from shb where name='$sender'";
    $result = $con-> query($sql);
    while($row = $result->fetch_assoc())
     {
...
//  Transcation DEatiled Stored in the DB
 ...
 $sql = "SELECT * from shb where name='$receiver'";
 $result = $con-> query($sql);
 while($row = $result->fetch_assoc())
  {
